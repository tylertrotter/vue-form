		...
		<template v-else-if="type.indexOf('section') > -1">
			...
			<div class="c-section-container c-cf">
				<h4 v-if="type === 'repeating-section'">
					<c-button v-if="type === 'repeating-section'" class="remove-section-button">
						<c-ex />
					</c-button>Item X
				</h4>
				<div class="c-shift">
			...
			<c-button v-if="type === 'repeating-section'" class="btn-primary add-section-button"><c-plus />Add Item</c-button>
		</template>
...
export default {
  ...
  props: ["colspan", "title", "error", "column", "help", "type", "required"]
};
</script>
